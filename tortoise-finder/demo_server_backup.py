    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        #validation-gallery {
            ...
            gap: 12px !important;
            min-height: 240px !important; /* prevents 0-height flashes */
            width: 100% !important;
...
<body>
    <div class="container">
        ...
        <div class="tab-navigation">
            <button class="tab-button active" onclick="switchTab('detection', this)">
                <i class="fas fa-search"></i>
                Detection
            </button>
            <button class="tab-button" onclick="switchTab('validation', this)">
                <i class="fas fa-check-double"></i>
                Validation
            </button>
            <button class="tab-button" onclick="switchTab('map', this)">
            ...
            <div class="validation-card">
                <div class="validation-header">
                    ...
                    <div class="validation-controls">
                        ...
                        <button class="btn btn-secondary" onclick="loadValidationImages()">
                            <i class="fas fa-sync-alt"></i>
                            Refresh
                        </button>
                ...
                <div class="gallery-container">
                    <div id="validation-gallery" class="gallery">
                        <div class="empty-state">
                            ...
                            <p>No images loaded for validation.</p>
                            <small>Run a detection or click refresh to load training images.</small>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        if (panel) panel.classList.add('active');    // <-- make visible first

        // layout-sensitive work AFTER visibility
        requestAnimationFrame(() => {
            if (tabName === 'validation') {
            loadValidationImagesScoped();
            }
            if (tabName === 'map' || tabName === 'gps') {
            if (!map) initializeMap();
            // ensure Leaflet/Mapbox recomputes size when tab becomes visible
        ...
        try {
            ...
            <div class="validation-item" data-tile-id="${item.tile_id}">
                <img loading="lazy" src="${item.thumb_url}" alt="${item.tile_id}">
        ...
        /* Map initialize (Leaflet) — supports #map or #gps-map */
        function initializeMap() {
        const el = document.getElementById('map') || document.getElementById('gps-map');
        if (!el) { console.error('Map element not found'); return; }
        map = L.map(el);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(map);
        map.setView([-0.6, -90.4], 9); // default Galápagos view
        }
        ...
        async function loadValidationImages() {
            try {
                ...
                const grid = document.getElementById('validation-gallery');
                if (!grid) {
                    console.error('Validation: gallery element not found');
                    return;
                }
                ...
                if (items.length === 0) {
                    grid.innerHTML = `
                        <div class="empty-state">
                            <i class="fas fa-images"></i>
                            <p>No images available for validation.</p>
                        </div>
                    `;
                    return;
                }
                ...
                grid.innerHTML = items.map(item => `
                    <div class="validation-item" data-tile-id="${item.tile_id}">
                        ...
                        <div class="validation-actions">
                            <button class="btn btn-verify" onclick="validateImage('${item.tile_id}', 'verified')">
                                <i class="fas fa-check"></i>
                                Verify
                            </button>
                            <button class="btn btn-reject" onclick="validateImage('${item.tile_id}', 'rejected')">
                                <i class="fas fa-times"></i>
                                Reject
                            </button>
                        </div>
                ...
                console.log(`Validation: DOM now has ${grid.querySelectorAll('.validation-item').length} validation-item nodes`);
        ...
        function displayValidationGallery(items) {
            const gallery = document.getElementById('validation-gallery');
            if (!gallery) {
                console.error('Validation: gallery element not found');
                return;
            }
            ...
            const html = items.map(item => {
                const status = validationData.get(item.tile_id) || 'pending';
                const statusClass = status !== 'pending' ? status : '';
                const statusText = status === 'verified' ? 'VERIFIED' : status === 'rejected' ? 'REJECTED' : '';
                
                return `
                    <div class="validation-item ${statusClass}" data-tile-id="${item.tile_id}">
                        <img src="${item.thumb_url}" alt="${item.tile_id}">
                        ${statusText ? `<div class="validation-status ${status}">${statusText}</div>` : ''}
                        <div class="validation-actions">
                            <button class="btn btn-verify" onclick="validateImage('${item.tile_id}', 'verified')">
                                <i class="fas fa-check"></i>
                                Verify
            ...
            const domCount = gallery.querySelectorAll('.validation-item').length;
            console.log('Validation: rendered', items.length, 'items into gallery');
            console.log('Validation: DOM now has', domCount, 'validation-item nodes');
        }
        ...
        function initializeMap() {
            ...
            // Load confirmed locations from the confirmed folder
                    ...
                    const markers = valid.map(p => {
                        const popupHtml = `
                            <div style="text-align: center;">
                                <strong><i class="fas fa-check-circle"></i> Confirmed Tortoise</strong><br>
                                <div style="margin:6px 0;">
                                    <small>Lat: ${p.lat.toFixed(6)}, Lon: ${p.lon.toFixed(6)}</small>
                                </div>
                                ${p.tile_id ? `<div style="margin:6px 0;"><a href="${p.image_url}" target="_blank" rel="noopener">Open full image</a></div>` : ''}
                                ${p.image_url ? `<img src="${p.image_url}" alt="${p.tile_id || 'Image'}" style="max-width:320px;height:auto;display:block;margin:4px auto;" />` : ''}
                            </div>`;
                        return L.marker([p.lat, p.lon]).bindPopup(popupHtml);
                    });
                    
                    const fg = L.featureGroup(markers).addTo(map);
                    map.fitBounds(fg.getBounds().pad(0.2));
                    verifiedMarkers.push(...markers);
                } else {
                    // Default Galápagos view if no GPS data
                    map.setView([-0.6, -90.4], 9);
                }
                updateMapStats();
            } catch (err) {
        ...
        function addMockVerifiedLocations() {
            // Add some mock verified tortoise locations as fallback
            const mockLocations = [
                { lat: -0.4, lon: -90.3, id: 'B002T-001', date: '2020-07-09', camera: 'B002T' },
                { lat: -0.6, lon: -90.4, id: 'B002T-002', date: '2020-07-11', camera: 'B002T' },
            ];
            ...
            mockLocations.forEach(location => {
                const marker = L.marker([location.lat, location.lon])
                    ...
                    .bindPopup(`
                        <div style="text-align: center;">
                            <strong><i class="fas fa-check-circle"></i> Mock Tortoise</strong><br>
                            <strong>ID:</strong> ${location.id}<br>
                            <strong>Date:</strong> ${location.date}<br>
                            <strong>Camera:</strong> ${location.camera}<br>
                            <small>Lat: ${location.lat.toFixed(5)}, Lon: ${location.lon.toFixed(5)}</small>
                        </div>
                    `);
        ...
        function addToMapData(tileId) {
            // In a real implementation, this would extract GPS from image EXIF
            // For demo, we'll add a random location near Galápagos
            const lat = -0.5 + (Math.random() - 0.5) * 0.4;
            const lon = -90.5 + (Math.random() - 0.5) * 0.4;
            ...
            if (map) {
                const marker = L.marker([lat, lon])
                    ...
                    .bindPopup(`
                        <div style="text-align: center;">
                            <strong><i class="fas fa-plus-circle"></i> Newly Verified</strong><br>
        ...
        function updateMapStats() {
            document.getElementById('verified-count').textContent = verifiedMarkers.length;
            document.getElementById('camera-count').textContent = new Set(verifiedMarkers.map(m => 'B002T')).size + 1; // Mock camera count
            document.getElementById('date-range').textContent = '2020-07-09 to 2020-07-16';
        }
        ...
        document.getElementById('validation-filter').addEventListener('change', loadValidationImages);
    </script>
</body>
</html>
"""
      ...
      <Point><coordinates>-90.1,-0.4,0</coordinates></Point>
            ...
            for fp in glob.glob(os.path.join(CONFIRMED_PATH, "*.*")):
                ...
                points.append({
                    "lat": lat, 
                    "lon": lon,
                    "tile_id": os.path.splitext(fn)[0] if lat is not None else None,
                    "image_url": f"/local_image/{urllib.parse.quote(fn)}" if lat is not None else None
                })
